...
SUBROUTINE chempot_lauerism(rismt, ierr)
  ...
  USE solvmol,   ONLY : get_nuniq_in_solVs, solVs, iuniq_to_isite, isite_to_isolV, isite_to_iatom
  !
  IMPLICIT NONE
  !
  ...
  DO iq = rismt%mp_site%isite_start, rismt%mp_site%isite_end
    ...
    rate       = rhov_left / rhov_right
    qv         = solVs(isolV)%charge(iatom)
    !
    CALL chempot_laue_for_a_site(rismt, ichempot, iiq, rate, qv, beta, usol)
    rismt%usol(iiq) = usol
    !
    CALL chempot_laue_for_a_site(rismt, CHEMPOT_GF, iiq, rate, qv, beta, usol)
...
!
!---------------------------------------------------------------------------
SUBROUTINE chempot_laue_for_a_site(rismt, ichempot, isite, rate, qv, beta, usol)
  !---------------------------------------------------------------------------
  !
  ...
  REAL(DP),        INTENT(IN)  :: rate  ! rhov(left) / rhov(right)
  REAL(DP),        INTENT(IN)  :: qv
  REAL(DP),        INTENT(IN)  :: beta
  REAL(DP),        INTENT(OUT) :: usol
  !
  IF (ichempot == CHEMPOT_HNC) THEN
    CALL chempot_laue_HNC_x(rismt, isite, rate, qv, beta, usol)
    !
  ELSE IF (ichempot == CHEMPOT_KH) THEN
    CALL chempot_laue_KH_x(rismt, isite, rate, qv, beta, usol)
    !
  ELSE IF (ichempot == CHEMPOT_GF) THEN
    CALL chempot_laue_GF_x(rismt, isite, rate, qv, beta, usol)
    !
  ELSE
    usol = 0.0_DP
  END IF
...
!
!---------------------------------------------------------------------------
SUBROUTINE chempot_laue_HNC_x(rismt, isite, rate, qv, beta, usol)
  !---------------------------------------------------------------------------
  !
...
!
!---------------------------------------------------------------------------
SUBROUTINE chempot_laue_KH_x(rismt, isite, rate, qv, beta, usol)
  !---------------------------------------------------------------------------
  !
...
!
!---------------------------------------------------------------------------
SUBROUTINE chempot_laue_GF_x(rismt, isite, rate, qv, beta, usol)
  !---------------------------------------------------------------------------
  !
...
!$omp parallel do default(shared) private(iz, jz, csr, clr, cr, hr) reduction(+:usol_)
    DO iz = 1, rismt%lfft%izleft_gedge
      ...
      IF (jz > 0) THEN
        csr = DBLE(rismt%csgz(jz, isite)) + rismt%cdza(isite) * rismt%cdzs(jz)
      ELSE
        csr = 0.0_DP
      END IF
