...
MODULE rism
  ...
  TYPE rism_type
    ...
    REAL(DP),    POINTER :: hsg0(:,:)    ! short-range total correlations when Gxy = 0 (Laue-RISM)
...
CONTAINS
  ...
  SUBROUTINE allocate_rism(rismt, itype, nsite, nsite_t, nr, nrzs, nrzl, ng, ngs, ngxy, lboth)
    ...
    ELSE IF (itype == ITYPE_LAUERISM) THEN
      ...
      IF ((nrzl * nsite) > 0) THEN
        ...
        ALLOCATE(rismt%hsg0(nrzl, nsite))
  ...
  SUBROUTINE deallocate_rism(rismt, lall)
    ...
    IF (ASSOCIATED(rismt%hsg0))      DEALLOCATE(rismt%hsg0)
  ...
  SUBROUTINE clean_rism_data(rismt)
    ...
    IF (ASSOCIATED(rismt%hsg0))     rismt%hsg0     = R_ZERO
