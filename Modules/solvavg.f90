...
CONTAINS
  ...
  SUBROUTINE solvavg_add_real(idata, integrate, zuv)
    ...
    IF (laue) THEN
      idx0  = nx1 * nx2 * lfft%dfft%ipp(lfft%dfft%mype + 1)
      i3min = lfft%dfft%ipp(lfft%dfft%mype + 1)
      i3max = lfft%dfft%npp(lfft%dfft%mype + 1) + i3min
    ELSE
      idx0  = nx1 * nx2 * dfft%ipp(dfft%mype + 1)
      i3min = dfft%ipp(dfft%mype + 1)
      i3max = dfft%npp(dfft%mype + 1) + i3min
    END IF
    !
    DO ir = 1, nnr
      !
