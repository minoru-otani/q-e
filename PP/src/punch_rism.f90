...
SUBROUTINE punch_rism(filplot)
  ...
  ! ... set FFT dimensions
  IF (.NOT. laue) THEN
    n1    = rism3t%cfft%dfftt%nr1
    n2    = rism3t%cfft%dfftt%nr2
    n3    = rism3t%cfft%dfftt%nr3
    nx1   = rism3t%cfft%dfftt%nr1x
    nx2   = rism3t%cfft%dfftt%nr2x
    nx3   = rism3t%cfft%dfftt%nr3x
    nz    = n3
    nxz   = nx3
    nfft  = nx1 * nx2 * nx3
  ELSE
  ...
  IF (ionode) THEN
    CALL plot_rism(TRIM(ADJUSTL(filplot)), title, ndata, nx1, nx2, nxz, n1, n2, nz, &
                 & nat, ntyp, ibrav, celldm, at, rism3t%cfft%gcutmt, laue, &
...
CONTAINS
  ...
  SUBROUTINE gather_3drism()
    ...
    END IF
    !
    IF (rism3t%nr < rism3t%cfft%dfftt%nnr) THEN
      CALL errore('punch_rism', 'nr < nnr', 1)
    END IF
    !
    IF (rism3t%ng < rism3t%cfft%ngmt) THEN
      CALL errore('punch_rism', 'ng < ngmt', 1)
    END IF
    !
    ! ... allocate working memory
    IF (rism3t%cfft%dfftt%nnr > 0) THEN
      ALLOCATE(aux(rism3t%cfft%dfftt%nnr))
      ALLOCATE(rhor(rism3t%cfft%dfftt%nnr))
      ALLOCATE(vpor(rism3t%cfft%dfftt%nnr))
    END IF
    ALLOCATE(rtmp(nfft))
    !
    ! ... set MPI-ranks
    ...
    !
    fft_rank = mp_rank(rism3t%cfft%dfftt%comm)
    !
    ...
    IF (my_group_id == io_group_id) THEN
      ...
      DO ig = 1, rism3t%cfft%ngmt
        aux(rism3t%cfft%nlt(ig)) = rism3t%rhog(ig)
      END DO
      IF (gamma_only) THEN
        DO ig = rism3t%cfft%gstart_t, rism3t%cfft%ngmt
          aux(rism3t%cfft%nltm(ig)) = CONJG(aux(rism3t%cfft%nlt(ig)))
        END DO
      END IF
      !
      IF (rism3t%cfft%dfftt%nnr > 0) THEN
        CALL invfft('Custom', aux, rism3t%cfft%dfftt)
      END IF
      !
      rhor = 0.0_DP
      DO ir = 1, rism3t%cfft%dfftt%nnr
        rhor(ir) = DBLE(aux(ir))
      END DO
      !
#if defined(__MPI)
      rtmp = 0.0_DP
      CALL gather_grid(rism3t%cfft%dfftt, rhor, rtmp)
#else
      rtmp = rhor
#endif
      IF (ionode) THEN
    ...
    IF (my_group_id == io_group_id) THEN
      ...
      END IF
      DO ig = 1, rism3t%cfft%ngmt
        aux(rism3t%cfft%nlt(ig)) = rism3t%vpot(ig)
      END DO
      IF (gamma_only) THEN
        DO ig = rism3t%cfft%gstart_t, rism3t%cfft%ngmt
          aux(rism3t%cfft%nltm(ig)) = CONJG(aux(rism3t%cfft%nlt(ig)))
...
#if defined(__MPI)
      rtmp = 0.0_DP
      CALL gather_grid(rism3t%cfft%dfftt, vpor, rtmp)
#else
      rtmp = vpor
#endif
      IF (ionode) THEN
...
#if defined(__MPI)
        rtmp = 0.0_DP
        CALL gather_grid(rism3t%cfft%dfftt, rism3t%gr(:, iiq), rtmp)
#else
        rtmp = rism3t%gr(1:rism3t%cfft%dfftt%nnr, iiq)
#endif
      ELSE
        owner_group_id = 0
        IF (fft_rank == rism3t%cfft%dfftt%root) THEN
          rtmp = 0.0_DP
        END IF
      END IF
      !
