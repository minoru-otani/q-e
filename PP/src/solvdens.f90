...
SUBROUTINE solvdens(filplot, lpunch)
  ...
  USE gvecs,          ONLY : gcutms, doublegrid, dual
  USE gvect,          ONLY : gcutm, ngm, nl, g
  ...
  ELSE IF (iflag  == 4) THEN
    ...
    !
    IF (output_format /= 0 .AND. &  ! gnuplot
      & output_format /= 1) THEN    ! contour.x
  ...
  DO isite = 1, nsite
    ...
    ELSE IF (iflag == 3) THEN
      ! ... 3D plot
      !
      ! ... open file for GOPENMOL's coordinates
      IF (output_format == 4) THEN
        IF (ionode) THEN
          ounit1   = ounit + 1
          fileout1 = TRIM(ADJUSTL(fileout0)) // '.xyz'
          OPEN(unit=ounit1, file=TRIM(ADJUSTL(fileout1)), status='unknown', form='formatted', iostat=ios)
          WRITE(stdout, '(5X,"Writing coordinates to file ",A)') TRIM(ADJUSTL(fileout1))
        END IF
        !
        CALL mp_bcast(ios,      ionode_id, intra_image_comm)
        CALL mp_bcast(fileout1, ionode_id, intra_image_comm)
        !
        IF (ios /= 0) THEN
          CALL errore('solvdens', 'error to open file: ' // TRIM(ADJUSTL(fileout1)), ABS(ios))
        END IF
      END IF
      !
      ! ... XCRYSDEN FORMAT
      ...
      ELSE IF (output_format == 6) THEN
        ...
        END IF
        !
      ! ... GOPENMOL OR XCRYSDEN FORMAT (fast-3D)
      ELSE IF (fast3d) THEN
        IF (ionode) THEN
        ...
        END IF
        !
      ! ... GOPENMOL OR XCRYSDEN FORMAT (standard)
      ELSE
        IF (nx <= 0 .OR. ny <= 0 .OR. nz <= 0) THEN
